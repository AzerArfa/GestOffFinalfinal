...
//GESTION OFFRE
    ...
    public ResponseEntity<Map<String, String>> createOffre(
        ...
        try {
            ...
            response.put("error", "Une offre de cette entreprise existe déjà pour cet appel d'offre.");
    ...
    @PostMapping("/valideroffre/{offreId}")
    public ResponseEntity<OffresValides> validateOffre(@PathVariable UUID offreId) {
        try {
            OffresValides validatedOffre = adminService.validateOffre(offreId);
            return ResponseEntity.ok(validatedOffre);
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(null); // Handle exceptions appropriately
        }
    }@GetMapping("/validatedoffre/{appelOffreId}")
    public ResponseEntity<OffresValides> getValidatedOffre(@PathVariable UUID appelOffreId) {
        Optional<OffresValides> validatedOffre = adminService.getValidatedOffreByAppelOffreId(appelOffreId);
        return validatedOffre.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
