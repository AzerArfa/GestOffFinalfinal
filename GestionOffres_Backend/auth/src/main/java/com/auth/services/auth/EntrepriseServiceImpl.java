...
public class EntrepriseServiceImpl implements EntrepriseService {
    ...
    @Override
    public List<EntrepriseDto> getEntreprisesByCategoryId(String idcategorie) {
        List<Entreprise> entreprises = entrepriseRepository.findByIdcategorie(idcategorie);
        return entreprises.stream().map(this::convertToDto).collect(Collectors.toList());
    }
    @Override
    @Transactional
    public void removeUserFromEntreprise(UUID entrepriseId, UUID userId) {
        Entreprise entreprise = entrepriseRepository.findById(entrepriseId).orElseThrow(() -> new ResourceNotFoundException("Entreprise not found"));
        User user = userRepository.findById(userId).orElseThrow(() -> new ResourceNotFoundException("User not found"));

        entreprise.removeUser(user);
