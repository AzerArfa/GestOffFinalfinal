	...
	@PostMapping("/login")
	  ...
	  public ResponseEntity<?> createAuthenticationToken(@RequestBody AuthenticationRequest authenticationRequest, HttpServletResponse response) {
	      ...
	      if (optionalUser.isPresent()) {
	          ...
	          if (!user.isEmailVerified()) {
	              ...
	              return ResponseEntity.status(HttpStatus.FORBIDDEN).body("E-mail non vérifié");
	          ...
	          if (authService.checkIfPasswordNeedsUpdate(user)) {
	              ...
	              return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Mise à jour du mot de passe requise.");
	  ...
	  public ResponseEntity<?> deleteUser(@PathVariable UUID id) {
	      try {
	          ...
	          if (!deleted) {
	              return ResponseEntity.badRequest().body("{\"message\":\"User could not be deleted.\"}");
	          }
	          ...
	          // Log the exception
	          e.printStackTrace();
	          return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("{\"message\":\"An error occurred while deleting the user.\"}");
	      }
	  }
	    ...
	    @GetMapping("entreprises/category/{id}")
	    public ResponseEntity<List<EntrepriseDto>> getEntreprisesByCategoryId(@PathVariable String id) {
	        List<EntrepriseDto> entreprises = entrepriseService.getEntreprisesByCategoryId(id);
	    ...
	    public ResponseEntity<?> requestResetPassword(@RequestParam String email) {
	        ...
	        String token = authService.generateResetPasswordToken();
	        user.setResetPasswordToken(token);
	        user.setResetpasswordtokenCreationDate(new Date());
	        userRepository.save(user);
	        authService.sendResetPassword(user);  // Ensure this method constructs the correct URL for the email
	    ...
	    public ResponseEntity<?> resetPassword(@RequestParam String token, @RequestBody ResetPasswordDto resetPasswordDto) {
	        ...
	        boolean isUnique = user.getPasswords().stream()
	            .noneMatch(p -> bCryptPasswordEncoder.matches(resetPasswordDto.getNewPassword(), p.getPassword()));
	        ...
	        newPassword.setPassword(bCryptPasswordEncoder.encode(resetPasswordDto.getNewPassword()));
...
public ResponseEntity<byte[]> downloadStatusDocument(@PathVariable UUID id) {
  ...
  //Notificationsssssssss
	    ...
	    public ResponseEntity<List<Notification>> getNotificationsByUser(@RequestParam List<UUID> categorieIds, @RequestParam List<UUID> entrepriseIds) {
	        List<Notification> notifications = notificationService.getNotificationsByUserEntreprises(categorieIds, entrepriseIds);
	    ...
	    @GetMapping("/count/entreprises")
	    public ResponseEntity<Long> getEntrepriseCount() {
	        long count = entrepriseService.getEntrepriseCount();
	        return ResponseEntity.ok(count);
	    }
	    @GetMapping("/count/users")
	    public ResponseEntity<Long> getUsersCount() {
	        long count = authService.getUsersCount();
	        return ResponseEntity.ok(count);
	    }
	    @DeleteMapping("/admin/{entrepriseId}/users/{userId}")
	    public ResponseEntity<Void> removeUserFromEntreprise(@PathVariable UUID entrepriseId, @PathVariable UUID userId) {
